...
- **Automatic sync, with nothing to set up.** The relay address is built into the app, so
  both phones keep themselves up to date: on open, when
  ...
  slot and reads the other's, so there are no conflicts to resolve. Each journey creates
  its own random room and key, so two couples on the same relay cannot see each other.
  Running your own relay instead is a one-line change plus [SETUP-SYNC.md](SETUP-SYNC.md).
